...
> Status: gap as of v0.2 (2026-04-29). Adoption: Tier B (on-demand —
> add baseline on first UI-touching PR per project, don't pre-schedule).
>
...
**Trigger-based, not date-based.** When a PR touches UI in a project
that doesn't yet have visual regression, that PR adds the baselines.

### Per-project initial setup (~半日)
...
AgentStars / CardWize / KanseiLink admin / Linksee Memory CLI も同じ
パターン: 5-10本のシーンを baseline 化、新シーンは PR で追加。

### CI integration (one-time, after first project's baselines exist)
